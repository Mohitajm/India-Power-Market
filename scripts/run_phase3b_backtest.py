"""
scripts/run_phase3b_backtest.py — Architecture v9_revised (v2)
"""
import pandas as pd, numpy as np, yaml, json, sys, argparse
...
def run_backtest(args):
    print("=" * 60)
    print("PHASE 3B: SOLAR+BESS BACKTEST -- Architecture v9_revised v2")
    print("=" * 60)
    ...
    if getattr(args, "day", None): dates = [args.day]
    elif getattr(args, "limit", None): dates = dates[:args.limit]
    ...
    for i, date in enumerate(dates):
        ...
        if res1["status"] != "Optimal":
            print(f"Stage 1 FAILED: {res1['status']}"); continue
        ...
        cap_net = float(np.sum(ev["block_captive_net"]))
        print(f"Net:Rs{net_rev:,.0f} Cap:Rs{cap_net:,.0f} SOC_end:{eod_soc:.4f}")

        dsm_serial = [{k: float(v) if isinstance(v, (float, np.floating)) else v
                       for k, v in d.items()} for d in ev["dsm_results"]]

        daily_out = {
            "date": date, "status": res1["status"],
            "p_max_mw": bp.p_max_mw, "solar_inverter_mw": bp.solar_inverter_mw,
            "soc_initial_mwh": SOC_TARGET,
            "soc_terminal_actual_mwh": eod_soc,
            "expected_revenue": res1["expected_revenue"],
            "net_revenue": net_rev, "captive_net_revenue": cap_net,
            "x_c": res1["x_c"], "x_d": res1["x_d"],
            "s_c_da": res1["s_c_da"], "s_cd_da": res1["s_cd_da"],
            "c_d_da": res1["c_d_da"],
            "captive_da": res1["captive_da"],
            "schedule_da": res1["schedule_da"], "setpoint_da": res1["setpoint_da"],
            "dam_schedule": res1["dam_schedule"],
            "solar_band_mask": res1.get("solar_band_mask", []),
            "y_c": ev["y_c"].tolist(), "y_d": ev["y_d"].tolist(),
            "s_c_rt": ev["s_c_rt"].tolist(), "s_cd_rt": ev["s_cd_rt"].tolist(),
        ...
        backtest_results.append({"target_date": date, "net_revenue": net_rev,
                                 "captive_net": cap_net, "soc_terminal": eod_soc})
